...
import { findFipeCode } from '@/lib/fipe/matcher';
import { POPULAR_MODELS_BR } from '@/lib/fipe/corrector';
import { getPricesByFipeCode, parseReferenceMonth } from '@/lib/fipe/client';
  ...
  try {
    ...
    for (const lead of enrichedLeads) {
      const isDuplicate = uniqueLeads.some((existing) => {
        // Mesmo cliente (nome) ou mesmo telefone de contato
        const sameClient = 
        ...
        if (sameClient) {
          // Compara os veículos limpando caracteres especiais
          ...
          // Se a similaridade do veículo for alta (>= 0.75) ou um contiver o outro
    ...
    (leads || []).forEach((lead: any) => {
      if (lead.cidade) {
        ...
        if (cleanCity) {
          // Capitaliza a primeira letra de cada palavra da cidade para ficar esteticamente elegante
