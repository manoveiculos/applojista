...
import { deduceBrandFromText } from '@/lib/parser/extractor';
...
export async function GET(request: Request) {
  try {
    ...
    try {
      // Deleta anúncios sem marca, modelo ou ano
      await supabaseAdmin
        .from('repassecentral')
        .delete()
      ...
      // Deleta anúncios explicitamente não identificados ou não informados
      await supabaseAdmin
        .from('repassecentral')
        .delete()
    ...
    // 2. Remove duplicados aproximados de forma inteligente (o mais barato prevalece)
    const finalOpps: any[] = [];
    for (const opp of opportunities) {
      const duplicateIndex = finalOpps.findIndex((existing) => {
        // Mesma marca, ano modelo e quilometragem muito próxima (tolerância de até 500 km por pequenas variações)
        const sameSpecs = 
          opp.brand === existing.brand &&
          opp.year_model === existing.year_model &&
          Math.abs(opp.km - existing.km) <= 500;

        if (sameSpecs) {
          // Compara os modelos limpando caracteres especiais
          ...
          // Se a similaridade for muito alta (>= 0.75) ou um modelo contiver o outro
      ...
      if (duplicateIndex === -1) {
        ...
        // Encontrou duplicado. Mantém o de menor preço de venda (ask_price)
        const existing = finalOpps[duplicateIndex];
        const oppPrice = Number(opp.ask_price) || 0;
        const existingPrice = Number(existing.ask_price) || 0;
        
        if (oppPrice > 0 && (existingPrice <= 0 || oppPrice < existingPrice)) {
          console.log(`[De-duplicação] Substituindo oferta existente do ${existing.model} (R$ ${existingPrice}) pela mais barata (R$ ${oppPrice}) de outro anúncio.`);
          finalOpps[duplicateIndex] = opp;
        }
      }
    }
