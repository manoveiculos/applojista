...
export default function AvaliacaoPage() {
  ...
  // Inteligência do Corretor
  const [correctionSuggestion, setCorrectionSuggestion] = useState<CorrectionSuggestion | null>(null);

  // Parâmetros de Precificação Lojista (Ajustáveis por Sliders)
  const [prepCost, setPrepCost] = useState<number>(2500); // custo de preparação padrão R$ 2500
  const [condition, setCondition] = useState<string>('bom'); // estado de conservação padrão

  // Margem de lucro computada dinamicamente com base nas ofertas históricas ou 15% por padrão
  const marginPercent = useMemo(() => {
    if (!similarOffers || similarOffers.length === 0) {
      return 15; // 15% automático quando não tiver histórico do carro
    }
    
    // Calcula a margem de cada anúncio no histórico em relação à sua FIPE de referência
    const margins = similarOffers.map(offer => {
      ...
      if (fipe > 0 && purchase > 0) {
        // Margem praticada é a diferença percentual (deságio) em relação à FIPE
    ...
    return Math.round(avg * 10) / 10; // Arredonda para uma casa decimal (ex: 15.6%)
  }, [similarOffers]);

  // Efeito para carregar parâmetros da URL e buscar automaticamente (integração de oportunidades)
  useEffect(() => {
  ...
  const handleSearch = async (e?: React.FormEvent, overrideBrand?: string, overrideModel?: string) => {
    ...
    try {
      const res = await fetch(
        `/api/avaliacao?brand=${encodeURIComponent(activeBrand)}&model=${encodeURIComponent(
          activeModel
      ...
      // Se a API retornar sugestão de correção de marca x modelo
      ...
      console.warn('[Fipe Search Warning]', err);
  ...
  // Função para aplicar a correção ortográfica/marca sugerida
  const handleApplyCorrection = async (suggestedBrand: string, suggestedModel: string) => {
    ...
    // 2. Atualiza os estados do input
    setBrand(formattedBrand);
    setModel(suggestedModel);
    setCorrectionSuggestion(null);

    // 3. Roda a busca automaticamente com os novos dados
    await handleSearch(undefined, suggestedBrand, suggestedModel);
  };

  // Função para selecionar um modelo da lista de ambiguidade
  const handleSelectOption = async (fipeCode: string) => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(
        `/api/avaliacao?brand=${encodeURIComponent(brand)}&model=${encodeURIComponent(
          model
      ...
      console.warn('[Fipe Select Option Warning]', err);
  ...
  // Cálculos Dinâmicos em Tempo Real (UseMemo)
  const precificacao = useMemo(() => {
    ...
    // Idade do carro (mínimo de 1 ano)
    ...
    if (kmDiff > 0) {
      // Excesso de km desvaloriza 0.05% por cada 1.000 km adicionais
      kmAdjustmentPercent = -(kmDiff / 1000) * 0.05;
      if (kmAdjustmentPercent < -15) kmAdjustmentPercent = -15; // teto -15%
    } else {
      // Baixo km valoriza 0.03% por cada 1.000 km economizados
      kmAdjustmentPercent = (Math.abs(kmDiff) / 1000) * 0.03;
      if (kmAdjustmentPercent > 8) kmAdjustmentPercent = 8; // teto +8%
    }

    // Ajuste por Estado de Conservação
    ...
    switch (condition) {
      case 'excelente':
        ...
        break;
      case 'bom':
        conditionAdjustmentPercent = -2;
        break;
      case 'regular':
        conditionAdjustmentPercent = -6;
        break;
      case 'ruim':
        conditionAdjustmentPercent = -15;
        break;
      default:
        conditionAdjustmentPercent = -2;
    }

    // Cálculo do Preço Estimado de Venda no Varejo (Mercado)
    const totalAdjustmentPercent = kmAdjustmentPercent + conditionAdjustmentPercent;
    const estimatedRetailPrice = baseFipe * (1 + totalAdjustmentPercent / 100);

    // Margem do Lojista (baseado no valor de venda)
    const marginAmount = estimatedRetailPrice * (marginPercent / 100);

    // Preço Recomendado de Compra
    const recommendedPurchasePrice = Math.max(estimatedRetailPrice - marginAmount - prepCost, 0);

    // Desconto sobre a FIPE
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 relative overflow-hidden flex flex-col">
      ...
      <header className="border-b border-zinc-900 bg-black/60 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-3 md:gap-5 text-[11px] md:text-xs font-bold">
            ...
            <span className="text-zinc-800">|</span>
            <Link href="/avaliacao" className="text-primary hover:text-primary transition-colors">
              Calculadora
            </Link>
            {unlocked && (
              <>
      ...
      {/* Main Container */}
      <main className="flex-1 max-w-7xl mx-auto px-6 py-10 w-full grid grid-cols-1 lg:grid-cols-12 gap-8 z-10">
          ...
          <div className="glass-panel border border-zinc-800 rounded-2xl p-6 flex flex-col gap-5">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-400 mt-0.5">Insira os dados para simulação da FIPE</p>
              </div>
            </div>
            ...
            <form onSubmit={(e) => handleSearch(e)} className="flex flex-col gap-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full mt-2 py-4 px-6 rounded-xl bg-primary hover:bg-primary/90 text-white font-bold text-sm transition-all flex items-center justify-center gap-2 group cursor-pointer glow-primary glow-primary-hover disabled:opacity-50"
              >
          ...
          <div className="glass-panel border border-zinc-900 rounded-2xl p-5 text-xs text-zinc-400 flex gap-3">
            ...
            <div>
              ...
              <ul className="list-disc list-inside space-y-1">
                ...
                <li>Km excedente: -0.05% a cada 1.000 km rodados a mais.</li>
        ...
        {/* Lado Direito: Resultados e Sliders */}
        <section className="lg:col-span-8 flex flex-col gap-6">
          
          {/* Caso 1: Aguardando Pesquisa */}
          {!searchDone && !loading && (
          ...
          {/* Caso 3: Sugestão de Correção Inteligente (Marca x Modelo trocado) */}
            ...
            <div className="glass-panel border border-amber-500/20 bg-amber-950/5 rounded-2xl p-6 md:p-8 flex flex-col gap-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mt-1">Parece que houve um equívoco ao preencher a marca do modelo procurado.</p>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-6 py-3.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-black font-extrabold text-xs transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-amber-500/10"
                >
          ...
          {/* Caso 4: Múltiplas Opções Encontradas (Opção para o cliente selecionar) */}
                ...
                {optionsList.map((opt) => (
                  ...
                  >
                    ...
                    <div className="text-right shrink-0 ml-4">
                      <span className="text-[9px] text-zinc-500 block uppercase">{opt.is_estimated ? 'Estimativa FIPE' : 'Referência FIPE'}</span>
                      <span className={`text-sm font-bold mt-0.5 block ${opt.is_estimated ? 'text-amber-400' : 'text-emerald-400'}`}>
          ...
          {/* Caso 5: Resultado Encontrado (Preenchido Direct ou pós-seleção) */}
          {searchDone && fipeData && precificacao && !loading && (
            <>
              ...
              <div className="glass-panel border border-zinc-800 rounded-2xl p-6 md:p-8 flex flex-col gap-6 relative overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-zinc-900">
                  <div>
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border bg-emerald-500/5 ${fipeData.is_estimated ? 'border-amber-500/20 text-amber-400 bg-amber-500/5' : 'border-emerald-500/20 text-emerald-400'}`}>
                      {fipeData.is_estimated ? 'Preço Estimado (FIPE Instável)' : `Fipe Oficial Identificada (${fipeData.fipe_code})`}
                    </span>
                    ...
                    {fipeData.is_estimated && (
                      <div className="mt-3.5 p-3.5 rounded-xl border border-amber-500/20 bg-amber-500/5 text-amber-400 text-xs flex items-start gap-2.5 max-w-xl">
                        <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
                        <span>
                          <strong>Preço Estimado Resiliente:</strong> A FIPE oficial está instável no momento. Estimamos este valor aplicando a valorização média do mercado de repasse ao modelo cadastrado mais próximo.
                        </span>
                      </div>
                    )}
                  </div>
                ...
                {/* Grid de Sliders e Controles do Lojista */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col gap-5">
                    ...
                    {/* Box Margem Inteligente */}
                    <div className="bg-zinc-950/60 border border-zinc-900 rounded-xl p-4 flex flex-col gap-3 relative overflow-hidden">
                      <div className="flex justify-between items-start">
                        <div>
                          <span className="text-zinc-400 text-sm font-semibold block">Margem de Lucro Aplicada</span>
                          <span className="text-[10px] text-zinc-500 block mt-1">
                            {similarOffers && similarOffers.length > 0 
                              ? `Média calculada sobre ${similarOffers.length} anúncios recentes`
                              : 'Estimada de forma padrão (Sem anúncios)'}
                          </span>
                        </div>
                        <div className="text-right">
                          ...
                          <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider block mt-1">
                            {similarOffers && similarOffers.length > 0 ? 'Histórica' : 'Padrão'}
                          </span>
                        </div>
                      </div>
                      ...
                      <p className="text-[10px] text-zinc-400 leading-normal">
                        {similarOffers && similarOffers.length > 0
                          ? 'A margem foi calculada com base no deságio médio praticado nos últimos anúncios deste modelo no banco de dados.'
                          : 'Como não há histórico de anúncios recentes para este modelo, a margem de segurança de 15% foi aplicada automaticamente.'}
                      </p>
                    </div>

                    {/* Slider Custos Preparação */}
                ...
                {/* Breakdown e Detalhamento da Matemática */}
                <div className="bg-zinc-950 border border-zinc-900 rounded-2xl p-5 flex flex-col gap-3.5">
                  <h4 className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Breakdown de Avaliação de Mercado</h4>
                  ...
                  <div className="flex flex-col gap-2.5 text-xs text-zinc-400">
                    ...
                    <div className="flex justify-between">
                      <span className="flex items-center gap-1">
                        Ajuste por Desgaste de KM
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-zinc-900 text-zinc-500">
                          Esperado: {precificacao.expectedKm.toLocaleString('pt-BR')} km
                        </span>
                      </span>
                    ...
                    <div className="flex justify-between">
                      <span>Estado de Conservação ({condition.toUpperCase()})</span>
                    ...
                    <div className="flex justify-between font-bold text-zinc-200">
                      <span>Valor Estimado de Venda (Varejo)</span>
                ...
                <div className="bg-primary/5 border border-primary/20 rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-400 mt-1">Este é o valor máximo sugerido para compra pelo repasse de forma a garantir sua margem líquida.</p>
              ...
              {/* Seção de Ofertas Similares Encontradas no Whatsapp (Evidência Física) */}
              <div className="glass-panel border border-zinc-900 rounded-2xl p-6 flex flex-col gap-5">
                <h3 className="font-bold text-white text-base flex items-center gap-2">
                  <TrendingDown className="w-5 h-5 text-primary" /> Ofertas Históricas Reais do Banco
                </h3>

                {similarOffers.length === 0 ? (
                  <p className="text-xs text-zinc-500 italic">Não foram encontradas ofertas anteriores no banco de dados para a mesma marca e termos deste modelo.</p>
                ) : (
                      ...
                      return (
                        <div key={offer.id} className="bg-zinc-950 border border-zinc-900 hover:border-zinc-800 rounded-xl p-4 flex flex-col justify-between gap-3 transition-colors">
                          <div className="flex justify-between items-start">
                            ...
                            {discountFipeNum !== null && (
                              <span className={`text-[9px] font-bold bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded ${discountFipeNum <= -10 ? 'text-lime-400' : 'text-emerald-400'}`}>
                                {discountFipeNum > 0 ? `+${discountFipeNum}%` : discountFipeNum < 0 ? `${discountFipeNum}%` : 'Fipe'}
                              </span>
                            )}
                          </div>
                          ...
                          <div className="flex justify-between items-end border-t border-zinc-900/60 pt-2 text-xs">
                            <div>
                              <span className="text-[9px] text-zinc-500 block uppercase">Preço Pedido</span>
                            ...
                            <div className="text-right">
                              <span className="text-[9px] text-zinc-500 block uppercase">Preço Líquido (Repasse)</span>
