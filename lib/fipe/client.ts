...
export function normalizeModelName(name: string): string {
  return name
    ...
    .replace(/\b(12v|16v|8v|v8|flex|gasol|diesel|aut|automatico|man|manual|mec|mecanico|turbo|2p|4p|4x4|4wd|awd)\b/g, '') // remove termos genéricos
    .replace(/\s+/g, ' ')
    .trim();
}
...
export function normalizeStrict(name: string): string {
  const semiClean = name
    ...
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/\b(12v|16v|8v|v8|flex|gasol|diesel|aut|automatico|man|manual|mec|mecanico|turbo|2p|4p|4x4|4wd|awd)\b/g, '');
...
export async function getFipePricesFromParallelum(
  ...
  try {
    ...
    const scoredModels = modelsList.map(m => {
      ...
      const inputWords = cleanInputModel.split(/\s+/).filter(Boolean);
      let matchedWordsCount = 0;
      inputWords.forEach(word => {
        if (cleanCandidateModel.includes(word)) {
          matchedWordsCount++;
        }
      });
      
      const wordMatchRatio = inputWords.length > 0 ? matchedWordsCount / inputWords.length : 0;

      let score = 0;
      if (wordMatchRatio === 1) {
        // Contém todas as palavras do input buscado
        score = 0.90 + (cleanInputModel.length / cleanCandidateModel.length) * 0.09;
      } else if (cleanCandidateModel.includes(cleanInputModel)) {
        score = 0.85 + (cleanInputModel.length / cleanCandidateModel.length) * 0.14;
      } else {
        const levScore = maxLen > 0 ? 1 - dist / maxLen : 0;
        score = levScore * 0.6 + wordMatchRatio * 0.4;
      }
