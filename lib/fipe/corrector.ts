...
export async function validateVehicleQuery(
  ...
  try {
    ...
    if (cacheMatches && cacheMatches.length > 0) {
      // Evita falsos positivos de substring (ex: buscar "X4" e casar com "4x4") garantindo correspondência de termo isolado
      const wordRegex = new RegExp(`(?:^|\\s|[^a-zA-Z0-9])${model.trim()}(?:$|\\s|[^a-zA-Z0-9])`, 'i');
      const filteredMatches = cacheMatches.filter(c => wordRegex.test(c.model));

      if (filteredMatches.length > 0) {
        // Conta as marcas associadas a esse modelo no cache
        const brandCounts: { [key: string]: number } = {};
        filteredMatches.forEach(c => {
