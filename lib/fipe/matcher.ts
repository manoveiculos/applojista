  ...
  try {
    ...
    const scoredCandidates = (candidates || []).map(c => {
      ...
      if (wordMatchRatio === 1) {
        ...
        similarity = 0.90 + (cleanInputModel.length / cleanCandidateModel.length) * 0.09;
      } else if (cleanCandidateModel.includes(cleanInputModel)) {
        similarity = Math.max(0.70, maxLen > 0 ? 1 - dist / maxLen : 0);
      } else {
        const levScore = maxLen > 0 ? 1 - dist / maxLen : 0;
        similarity = levScore * 0.6 + wordMatchRatio * 0.4;
      }
